...
var (
	...
	namespaceConfigTypeMap = map[string]string{}
	// 默认创建Agollo的Option
...
func SetConfigType(ct string, namespace string) {
	namespaceConfigTypeMap[namespace] = ct
}
...
func marshalSettings(configType string, configs map[string]interface{}) ([]byte, error) {
	if configs == nil || len(configs)<1 {
		return nil, errors.New("marshalSettings configs is nill")
	}
...
func getConfigType(namespace string) string {
	...
	configType, ok := namespaceConfigTypeMap[namespace]
	if !ok {
